...
export function isValidEntityType(
  entity: string,
): entity is (typeof VALID_ENTITIES)[number] {
...
export function isValidOperation(
  operation: string,
): operation is (typeof VALID_OPERATIONS)[number] {
...
export function validateQueueItemInput(input: CreateQueueItemInput): void {
  ...
  if (!input.operation || typeof input.operation !== "string") {
    throw new QueueValidationError(
      "Operation type is required and must be a string",
    );
  }
...
export function validateQueueItem(item: SyncQueueItem): void {
  ...
  if (!item.timestamp || typeof item.timestamp !== "string") {
    throw new QueueValidationError(
      "Timestamp is required and must be an ISO string",
    );
  }
